...
assoc(formula= Input+Structure~Fit.factor,data=components[,],shade=TRUE,gp=shading_Friendly,labeling_args = list(rot_labels = c(left = 90,top=90,bottom=90,right = 0)),gp_labels=gpar(fontsize=6),offset_labels = c(bottom = 1), margins = c(bottom = 2,right=5))
